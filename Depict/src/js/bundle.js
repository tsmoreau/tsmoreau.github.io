...
createArt: function(event){
  ...
  reader.onloadend = () => {
    ...
    return adoptionInstance.convert_art(name,desc,image,parseInt(price), {from: account});
          ...
          for (i = 0; i < arts.length; i++) {
            collectiblesinstance.getArt.call(arts[i]).then(function(fetched_art){
              if (fetched_art !== '0x0000000000000000000000000000000000000000') {
                ...
                petTemplate.find('img').attr('src', fetched_art[2]);
                petTemplate.find('.pet-breed').text(fetched_art[3]);
                petTemplate.find('.pet-age').text(fetched_art[4]);
